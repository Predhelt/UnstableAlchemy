...
extends Button
## Transaction ID that is stored as reference for the parent character_shop list of transactions.
var transaction_id : int = -1 
## UI Scene of an item in the transaction.
var shop_item_scene = preload("./shop_item.tscn")
## Sends signal to the character shop that a transaction was attempted.
...
## Triggers when the transaction is pressed, emits signal to character shop with this transaction's ID.
